...
//			map.FlyToCountry ("India");
        ...
        #region RiseOfSocietyCode
        GameObject movementLine = null;
        float drawFrequency = .075f;
        float timer = 0;
        Unit selectedUnit = null;
        Vector3 oldMapCurser; //Added this to try to help reduce redraws and flicking

        //Inline Function the math library only has a pow function which is inefficent for integer powers
        System.Func<float, float> Square = (x) => { return x * x; }; 

        void Update()
        {
            //Adds a unit at the mouse cursers location on the globe
            if (Input.GetKeyDown("u"))
            {
                AddMarkerUnitAtMouseCurser();
            }

            if(Input.GetMouseButtonDown(1))
            {
                Debug.Log("Mouse Button 1 down at time = " + Time.fixedTime);
            }
            ...
            if(Input.GetMouseButtonDown(0))
            {
                ...
                {
                    Vector3 sphereCursorLocation = toSphereCoords(map.cursorLocation);
                    Vector3 sphereSelectedUnitPosition = toSphereCoords(selectedUnit.bottomCube.transform.position);
                    //Something weird with the distance calculation, its intermittently giving me bad distances
                    //if (map.calc.Distance(sphereCursorLocation.x, sphereCursorLocation.y, sphereSelectedUnitPosition.x, sphereSelectedUnitPosition.y) <= selectedUnit.GetComponent<Unit>().distancePerTurn)
                    if (Distance(sphereSelectedUnitPosition, sphereCursorLocation, false) <= selectedUnit.distancePerTurnRadians)
                    {
                        
                        MoveUnitToMouseCurser(selectedUnit);
                        selectedUnit.DestroyMovementCircle();
                        Destroy(movementLine);
                        selectedUnit = null;
                        movementLine = null;
                    }
                ...
                { 
                    selectedUnit = LocateUnitUnderCurser();
                    if (selectedUnit != null)
                    {
                        AddMovementCircle(selectedUnit.bottomCube.transform.position);
                        movementLine = AddSphericalLine(selectedUnit.transform.position, map.cursorLocation, Color.black, 0f, 0.0025f);
                        timer = 0f;
                        oldMapCurser = map.cursorLocation;
                    }
                }
            }
            
            if(timer > drawFrequency && movementLine != null && selectedUnit != null && oldMapCurser != map.cursorLocation)
            {
                timer = 0f;

                //map.ChangeLineEndPoint(movementLine, map.cursorLocation); //This would get rid of the flickering and allow for a higher drawFrequency, but it isn't working.
                GameObject oldLine = movementLine;
                movementLine = AddSphericalLine(selectedUnit.transform.position, map.cursorLocation, Color.black, 0f, 0.0025f);
        ...
        void MoveUnitToMouseCurser(Unit unit)
        {
            map.AddMarker(unit.gameObject, map.cursorLocation, 0.01f);
            //unit.transform.position = map.cursorLocation;
        }

        void AddMovementCircle(Vector3 cartesianCenter)
        {
            //METHOD1
            //If Kronnet fixes the code for drawing circles this problem disappears.
            //AddMarker(MARKER_TYPE type, Vector3 sphereLocation, float kmRadius, float ringWidthStart, float ringWidthEnd, Color color)
            // Draw a beveled circle

            /*float km = 300f; // Circle with a radius of (500...1000) km
            map.AddMarker(MARKER_TYPE.CIRCLE, cartesianCenter, km, 0.975f, 1.0f, Color.red);
            map.AddMarker(MARKER_TYPE.CIRCLE, cartesianCenter, km, 0, 0.975f, Color.magenta);
            selectedUnit.transform.position = cartesianCenter;*/

            


            GameObject movementCircle = Instantiate(Resources.Load<GameObject>("Prefabs/MovementCircle"));
            movementCircle.transform.position = cartesianCenter;
            movementCircle.transform.localScale = new Vector3(.1f, .1f, .1f);
            movementCircle.transform.LookAt(map.transform.position);

            selectedUnit.GetComponent<Unit>().movementCircle = movementCircle;
            //movementCircle.transform.Translate(0, 0, 1);

            //METHOD2
            //This should be working, but because the circle point isnt moving with the circle the ray isn't hitting the earth.
            /*
            Vector3 circlePoint = movementCircle.GetComponent<MovementCircle>().circlePoint.transform.position;
            RaycastHit hit;
            if (Physics.Linecast(circlePoint, map.transform.position, out hit))
            {
                float magnitudeIntoEarth = Vector3.Distance(hit.transform.position, circlePoint)*1f;
                Vector3 normal = movementCircle.transform.position - Vector3.Project(circlePoint, map.transform.position); //Vector pointing towards the globe
                //normal.Normalize();
                normal *= magnitudeIntoEarth;
                movementCircle.transform.position -= normal;
                Debug.Log("circlePoint.transform.position: " + circlePoint);
                Debug.Log("hit.transform.position: " + hit.transform.position);
                Debug.Log("magnitudeIntoEarth: " + magnitudeIntoEarth);
                ...
                //Method3
                //Try taking a section of the pole and rotating it to get the need position
                //Not sure why this method didn't work, its a bit complicated though.
                /*Debug.Log("cartesianCenter: " + cartesianCenter.ToString());
                float radiusCircle = .001f;//.05f
                GameObject movementCircle = Instantiate(Resources.Load<GameObject>("Prefabs/MovementCircle"));
                //movementCircle.transform.localScale = new Vector3(2f*radiusCircle, 2f * radiusCircle, 2f * radiusCircle);
                movementCircle.transform.position = new Vector3(radiusCircle, 0.0f, -Mathf.Sqrt(Mathf.Pow(map.transform.lossyScale.x/2.0f, 2.0f) - Mathf.Pow(radiusCircle,2.0f)));
                movementCircle.transform.localEulerAngles = new Vector3(180f*(Mathf.PI - toSphereCoords(cartesianCenter).x)/Mathf.PI, 0f, (180f/Mathf.PI) * toSphereCoords(cartesianCenter).y);*/
                /*
                RaycastHit hit;
                if (Physics.Linecast(map.transform.position, selectedUnit.transform.position, out hit))
                {
                    GameObject movementCircle = Instantiate(Resources.Load<GameObject>("Prefabs/MovementCircle"));
                    movementCircle.transform.position = cartesianCenter;

                    float radiusD = map.calc.Distance(selectedUnit.transform.position, hit.transform.position);
                    movementCircle.transform.lossyScale.Set(radiusD, radiusD, radiusD);
        ...
        //May be redundant
        public Vector3 toSphereCoords(Vector3 cartesianPosition)
        {
            float latitude = Mathf.Asin(cartesianPosition.y * 2.0f);
            float longitude = Mathf.Atan2(cartesianPosition.x, cartesianPosition.z);
            float radius = .5f;

            //Could also try using 
            //Poly2Tri.PolygonPoint sphereCoords = map.GetLatLonFromSpherePoint(cartesianPosition);

            return new Vector3(latitude, longitude, radius);
        }

        Unit LocateUnitUnderCurser()
        {
            ...
            if (Physics.Raycast(ray, out hit))
            {
                ...
                return hitObject.GetComponent<Unit>();
            }

            return null;
        }

        float Distance(Vector3 point1, Vector3 point2, bool cartesianPoints = true)
        {
            float centralAngle;
            const float earthRadius = 1f;
            
            if (cartesianPoints)
            {
                point1 = toSphereCoords(point1);
                point2 = toSphereCoords(point2);
            }

            centralAngle = 2f * Mathf.Asin(Mathf.Sqrt(Square(Mathf.Sin((point2.x - point1.x) / 2f)) + Mathf.Cos(point1.x) * Mathf.Cos(point2.x) * Square(Mathf.Sin((point2.y - point1.y) / 2f))));
